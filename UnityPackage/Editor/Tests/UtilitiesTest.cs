...
namespace RhythmGameUtilities.Tests
{
    ...
    public class UtilitiesTest
    {
        ...
        public void TestCalculateNoteHitAccuracy()
        {
            ...
            Assert.That(Utilities.CalculateNoteHitAccuracy(ref note, buffer, note.Position - buffer), Is.EqualTo(0));
            Assert.That(Utilities.CalculateNoteHitAccuracy(ref note, buffer, note.Position - buffer / 2), Is.EqualTo(0.5f));
            Assert.That(Utilities.CalculateNoteHitAccuracy(ref note, buffer, note.Position + buffer), Is.EqualTo(0));
            Assert.That(Utilities.CalculateNoteHitAccuracy(ref note, buffer, note.Position + buffer / 2), Is.EqualTo(0.5f));
